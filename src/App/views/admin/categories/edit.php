...
<section class="max-w-2xl mx-auto my-12 p-4 bg-white shadow-md border border-gray-200 rounded">
    ...
    <?php if (!empty($_SESSION['CRUDMessage'])) : ?>
        <div class="d-flex align-items-center text-white rounded p-1 my-2 <?php echo (substr($_SESSION['CRUDMessage'], 0, 5) === 'Error') ? "bg-danger" : "bg-success" ?>">
            ...
            <div class="p-2">
                <a class="link-light link-opacity-100 link-opacity-50-hover text-decoration-none" href="/admin/category/<?php echo $category->id ?>">X</a>
            </div>
        </div>
    ...
    <form method="POST" class="grid grid-cols-1 gap-6">
        ...
        <label class="block">
            ...
            <input value="<?php echo $category->name ?>" name="category" type="text" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" placeholder="" />
            <?php if (array_key_exists('category', $errors)) : ?>
                <div class="bg-gray-100 mt-2 p-2 text-red-500"><?php echo ($errors['category'][0]); ?></div>
            <?php endif; ?>
        </label>
        <p>Created: <?php echo date("d/m/Y", strtotime($category->created_at)); ?> | Updated: <?php echo date("d/m/Y", strtotime($category->updated_at)); ?></p>
    ...
    <div class="d-grid mt-2 gap-2">
        <a href="/admin/category" class="btn btn-primary" role="button">Back</a>
    </div>
